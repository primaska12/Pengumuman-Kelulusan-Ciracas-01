...
async function readDB(): Promise<Student[]> {
  let parsedData: Student[] | null = null;
  if (pool) {
    try {
      ...
      console.error('Error reading students from DB:', err);
      // Wait we shouldn't overwrite if it errored here.
    }
  } else {
    try {
  ...
  // Since it failed to retrieve, we don't automatically overwrite DB. 
  // The system just starts with default data in memory if nothing exists. 
  // It will be written next time they add a student. But let's write it only 
  // if NOT pool because for PG it might be a cold start query fail.
  // Actually, we'll just not write it to be safe.
  try {
     // If not pool, write it so local development works smoothly
     if (!pool) await writeDB(defaultData);
  } catch(e) {}
...
async function startServer() {
  ...
  if (pool) {
    try {
      ...
      console.error('Error creating PG table:', err);
    }
  }
  ...
  app.post('/api/admin/students', authenticateAdmin, async (req, res) => {
    const { nama, nisn, tempatLahir, tglLahir, nilaiMatematika, nilaiBIndonesia, noPeserta, kelas, sekolah, status, tahunAjaran, isPranked } = req.body;
    try {
      const students = await readDB();
    ...
    try {
      ...
      for (const item of newStudents) {
        ...
        students.push({
          ...
          nilaiMatematika: item.nilaiMatematika,
          nilaiBIndonesia: item.nilaiBIndonesia,
  ...
  app.put('/api/admin/students/:id', authenticateAdmin, async (req, res) => {
    ...
    try {
      ...
      students[index] = {
        ...students[index],
        nama, nisn, tempatLahir, tglLahir, nilaiMatematika, nilaiBIndonesia, noPeserta, kelas, sekolah,
        status: status || 'Lulus',
  ...
  app.put('/api/admin/students/prank/batch', authenticateAdmin, async (req, res) => {
    const { studentIds, isPranked } = req.body;
    if (!Array.isArray(studentIds)) return res.status(400).json({ error: 'Invalid payload' });

    try {
      const students = await readDB();
      const updatedStudents = students.map(s => {
        if (studentIds.includes(s.id)) {
          return { ...s, isPranked: !!isPranked };
        }
        return s;
      });
