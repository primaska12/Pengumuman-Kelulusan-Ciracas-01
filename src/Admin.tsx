...
export default function Admin() {
  ...
  const [formData, setFormData] = useState<Partial<Student>>({
    nama: '', nisn: '', tempatLahir: '', tglLahir: '', noPeserta: '', kelas: '', sekolah: 'SDN CIRACAS 01', status: 'Lulus', tahunAjaran: '', isPranked: false
  });
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (continueAdding && !isEditing) {
          setFormData({ nama: '', nisn: '', tempatLahir: '', tglLahir: '', noPeserta: '', kelas: '', sekolah: 'SDN CIRACAS 01', status: 'Lulus', tahunAjaran: tahunAjaranAktif });
  ...
  const handleExportPDF = () => {
    ...
    const tableData = activeStudents.map(s => [
      s.nama, s.nisn, s.tempatLahir || '', s.tglLahir || '', s.kelas, s.noPeserta, s.status, s.tahunAjaran
    ]);

    autoTable(doc, {
      head: [['Nama', 'NISN', 'Tmpt Lahir', 'Tgl Lahir', 'Kelas', 'No Peserta', 'Status', 'Tahun Ajaran']],
      body: tableData,
      startY: 20,
      styles: { fontSize: 8 },
              ...
              <div className="animation-fade-in flex flex-col min-h-0 bg-white rounded-[20px] border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] overflow-hidden">
                ...
                {isAdding && (
                  <div className="p-4 bg-blue-50/50 border-b border-blue-100">
                    <div className="bg-white p-4 rounded-xl shadow-sm border border-blue-100 w-full overflow-x-auto min-w-[1000px]">
                      <div className="flex gap-4 items-center">
                        <input type="text" ref={nameInputRef} autoFocus placeholder="Nama Lengkap" value={formData.nama} onChange={e=>setFormData({...formData, nama: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="flex-[2] min-w-[150px] bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all" />
                        <input type="text" placeholder="NISN" value={formData.nisn} onChange={e=>setFormData({...formData, nisn: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="flex-1 min-w-[100px] bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all font-mono" />
                        <input type="text" placeholder="Tmpt Lahir" value={formData.tempatLahir || ''} onChange={e=>setFormData({...formData, tempatLahir: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="flex-1 min-w-[100px] bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all" />
                        ...
                        <input type="text" placeholder="Kelas" value={formData.kelas} onChange={e=>setFormData({...formData, kelas: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="w-20 bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all" />
                        <select value={formData.status} onChange={e=>setFormData({...formData, status: e.target.value})} onKeyUp={(e)=>e.key==='Enter' && handleSave(true)} onKeyDown={(e) => {if(e.key==='Enter') e.preventDefault();}} className="w-28 bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all font-medium text-slate-700 cursor-pointer">
                      ...
                      {paginatedStudents.map(student => {
                        ...
                        return (
                          <tr key={student.id} className="hover:bg-slate-50/70 transition-colors group">
                            {editingThis ? (
                              <td colSpan={9} className="p-3 bg-blue-50/30">
