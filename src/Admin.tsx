...
export default function Admin() {
  ...
  const [formData, setFormData] = useState<Partial<Student>>({
    nama: '', nisn: '', tempatLahir: '', tglLahir: '', nilaiMatematika: '', nilaiBIndonesia: '', noPeserta: '', kelas: '', sekolah: 'SDN CIRACAS 01', status: 'Lulus', tahunAjaran: '', isPranked: false
  });
    ...
    try {
      ...
      if (res.ok) {
        ...
        if (continueAdding && !isEditing) {
          setFormData({ nama: '', nisn: '', tempatLahir: '', tglLahir: '', nilaiMatematika: '', nilaiBIndonesia: '', noPeserta: '', kelas: '', sekolah: 'SDN CIRACAS 01', status: 'Lulus', tahunAjaran: tahunAjaranAktif });
  ...
  const handleTogglePrankBatch = async (isPranked: boolean) => {
    if (selectedStudents.length === 0) return;
    
    try {
      const res = await fetch('/api/admin/students/prank/batch', {
        method: 'PUT',
        headers: { 
          'Content-Type': 'application/json',
          Authorization: `Bearer ${token}` 
        },
        body: JSON.stringify({ studentIds: selectedStudents, isPranked })
      });
      
      if (res.ok) {
        showToast(`Berhasil ${isPranked ? 'menandai' : 'menghapus'} status Prank.`);
        fetchStudents();
        setSelectedStudents([]);
      } else {
  ...
  const handleExportExcel = () => {
    const dataToExport = activeStudents.map(s => ({
      ...
      'Nomor Peserta': s.noPeserta,
      'Matematika': s.nilaiMatematika || '',
      'B.Indo': s.nilaiBIndonesia || '',
      'Kelas': s.kelas,
      'Status': s.status,
  ...
  const handleExportPDF = () => {
    ...
    const tableData = activeStudents.map(s => [
      s.nama, s.nisn, s.tempatLahir || '', s.tglLahir || '', s.noPeserta, s.nilaiMatematika || '', s.nilaiBIndonesia || '', s.kelas, s.status, s.tahunAjaran
    ]);

    autoTable(doc, {
      head: [['Nama', 'NISN', 'Tmpt Lahir', 'Tgl Lahir', 'No Peserta', 'Matematika', 'B.Indo', 'Kelas', 'Status', 'Tahun Ajaran']],
      body: tableData,
      startY: 20,
      styles: { fontSize: 8 },
  ...
  const handleDownloadTemplate = () => {
    const ws = XLSX.utils.json_to_sheet([{
      ...
      'Tgl Lahir': '15-08-2008',
      'Nomor Peserta': '01-001-001-2',
      'Matematika': '85',
      'B.Indo': '90',
      'Kelas': 'VI - A',
      'Status': 'Lulus',
    ...
    reader.onload = async (evt) => {
      try {
        ...
        const formattedData = data.map(row => ({
          nama: row['Nama'] || row['Nama Siswa'] || row['nama'] || '',
          nisn: String(row['NISN'] || row['nisn'] || ''),
          tempatLahir: row['Tempat Lahir'] || row['tempatLahir'] || '',
          tglLahir: row['Tgl Lahir'] || row['tglLahir'] || row['Tanggal Lahir'] || '',
          nilaiMatematika: String(row['Matematika'] || row['matematika'] || row['nilaiMatematika'] || ''),
          nilaiBIndonesia: String(row['B.Indo'] || row['b.indo'] || row['Bahasa Indonesia'] || row['nilaiBIndonesia'] || ''),
          kelas: row['Kelas'] || row['kelas'] || '',
          noPeserta: String(row['Nomor Peserta'] || row['No Peserta'] || row['noPeserta'] || row['no_peserta'] || ''),
  ...
  const startAdding = () => {
    if (!tahunAjaranAktif) {
      alert("Pilih Tahun Ajaran terlebih dahulu untuk menambah data.");
      return;
    }
    setActiveTab('data');
    setIsAdding(true);
            ...
            {activeTab === 'data' && (
              <div className="animation-fade-in flex flex-col min-h-0 bg-white rounded-[20px] border border-slate-100 shadow-[0_4px_20px_rgb(0,0,0,0.03)] overflow-hidden">
                ...
                {selectedStudents.length > 0 && (
                  <div className="p-3 bg-blue-50/80 border-b border-blue-100 flex items-center justify-between animate-in fade-in slide-in-from-top-2">
                    <div className="flex items-center gap-3">
                      <span className="bg-blue-600 text-white w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold">
                        {selectedStudents.length}
                      </span>
                      <span className="text-sm font-semibold text-blue-900">Siswa Terpilih</span>
                    </div>
                    <div className="flex gap-2">
                      <button 
                        onClick={() => handleTogglePrankBatch(true)}
                        className="px-3 py-1.5 bg-amber-100 hover:bg-amber-200 text-amber-800 border border-amber-200 rounded-lg text-sm font-medium transition-colors"
                      >
                        Tandai Prank
                      </button>
                      <button 
                        onClick={() => handleTogglePrankBatch(false)}
                        className="px-3 py-1.5 bg-white hover:bg-slate-50 text-slate-700 border border-slate-200 rounded-lg text-sm font-medium transition-colors"
                      >
                        Hapus Prank
                      </button>
                    </div>
                ...
                {/* Adding Row Banner */}
                {isAdding && (
                  <div className="p-4 bg-blue-50/50 border-b border-blue-100 mb-6 rounded-xl">
                    <div className="bg-white p-6 rounded-xl shadow-sm border border-blue-100 flex flex-col gap-4">
                      
                      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-semibold text-slate-600 uppercase">Nama Lengkap</label>
                          <input type="text" ref={nameInputRef} autoFocus placeholder="Nama Lengkap" value={formData.nama} onChange={e=>setFormData({...formData, nama: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="w-full bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all" />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-semibold text-slate-600 uppercase">NISN</label>
                          <input type="text" placeholder="NISN" value={formData.nisn} onChange={e=>setFormData({...formData, nisn: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="w-full bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all font-mono" />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-semibold text-slate-600 uppercase">No. Peserta</label>
                        ...
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-semibold text-slate-600 uppercase">Tempat Lahir</label>
                          <input type="text" placeholder="Tmpt Lahir" value={formData.tempatLahir || ''} onChange={e=>setFormData({...formData, tempatLahir: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="w-full bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all" />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-semibold text-slate-600 uppercase">Tgl Lahir</label>
                          <input type="text" placeholder="Tgl Lahir" value={formData.tglLahir || ''} onChange={e=>setFormData({...formData, tglLahir: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="w-full bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all font-mono" />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-semibold text-slate-600 uppercase">Nilai Matematika</label>
                          <input type="text" placeholder="Matematika" value={formData.nilaiMatematika || ''} onChange={e=>setFormData({...formData, nilaiMatematika: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="w-full bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all" />
                        </div>
                        <div className="flex flex-col gap-1.5">
                          <label className="text-xs font-semibold text-slate-600 uppercase">Nilai B.Indo</label>
                          <input type="text" placeholder="B.Indo" value={formData.nilaiBIndonesia || ''} onChange={e=>setFormData({...formData, nilaiBIndonesia: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(true)} className="w-full bg-slate-50 border border-slate-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 rounded-lg px-3 py-2 text-sm outline-none transition-all" />
                        </div>
                      </div>
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-2 pt-4 border-t border-slate-100">
                        <div className="flex items-center gap-4 flex-wrap">
                          <div className="flex flex-col gap-1.5">
                            <label className="text-xs font-semibold text-slate-600 uppercase">Status Kelulusan</label>
                          ...
                          <label className="flex items-center gap-2 text-sm text-slate-600 font-medium whitespace-nowrap cursor-pointer hover:text-slate-800 lg:top-1/2 mt-6">
                             <input type="checkbox" checked={!!formData.isPranked} onChange={e=>setFormData({...formData, isPranked: e.target.checked})} className="w-4 h-4 rounded text-blue-600 border-slate-300 focus:ring-blue-500" />
                             Mode Prank
                          </label>
                          <div className="text-xs text-blue-500 font-medium lg:self-end mb-2.5 bg-blue-50 border border-blue-100 px-3 py-1.5 rounded-lg hidden xl:block">
                            💡 Tekan ENTER di kolom status untuk otomatis simpan
                          </div>
                        </div>
                        
                        <div className="flex gap-2 shrink-0 flex-wrap justify-end">
                          <button onClick={() => setIsAdding(false)} className="px-4 py-2 bg-white hover:bg-slate-50 border border-slate-200 text-slate-600 hover:text-slate-800 rounded-lg transition-colors font-medium text-sm flex gap-2 items-center">Batal</button>
                          <button onClick={() => handleSave(true)} className="px-4 py-2 bg-blue-50 hover:bg-blue-100 text-blue-700 border border-blue-200 rounded-lg shadow-sm font-medium text-sm transition-all flex items-center gap-2">Simpan & Tambah Lagi</button>
                          <button onClick={() => handleSave(false)} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg shadow-sm font-medium text-sm transition-all flex items-center gap-2"><Check className="w-4 h-4"/> Simpan Tutup</button>
                        </div>
                      </div>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-left min-w-[1400px] border-collapse">
                    <thead>
                      <tr className="bg-white border-b border-slate-200 text-[11px] sm:text-xs font-bold uppercase tracking-widest text-slate-500">
                      ...
                      {paginatedStudents.map(student => {
                        ...
                        return (
                          <tr key={student.id} className="hover:bg-slate-50/70 transition-colors group">
                            {editingThis ? (
                              <td colSpan={11} className="p-3 bg-blue-50/30">
                                <div className="flex gap-4 items-center pl-3 pr-3">
                                  <input type="text" value={formData.nama} onChange={e=>setFormData({...formData, nama: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(false)} className="flex-[2] min-w-[150px] bg-white border border-blue-300 focus:border-blue-500 focus:ring-1 rounded-md px-3 py-1.5 text-sm outline-none font-medium" autoFocus/>
                                  <input type="text" value={formData.nisn} onChange={e=>setFormData({...formData, nisn: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(false)} className="flex-1 min-w-[100px] bg-white border border-blue-300 focus:border-blue-500 focus:ring-1 rounded-md px-3 py-1.5 text-sm outline-none font-mono" />
                                  <input type="text" placeholder="Tmpt Lahir" value={formData.tempatLahir || ''} onChange={e=>setFormData({...formData, tempatLahir: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(false)} className="flex-1 min-w-[100px] bg-white border border-blue-300 focus:border-blue-500 focus:ring-1 rounded-md px-3 py-1.5 text-sm outline-none" />
                                  <input type="text" placeholder="Tgl Lahir" value={formData.tglLahir || ''} onChange={e=>setFormData({...formData, tglLahir: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(false)} className="flex-1 min-w-[100px] bg-white border border-blue-300 focus:border-blue-500 focus:ring-1 rounded-md px-3 py-1.5 text-sm outline-none font-mono" />
                                  <input type="text" value={formData.noPeserta} onChange={e=>setFormData({...formData, noPeserta: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(false)} className="flex-1 min-w-[120px] bg-white border border-blue-300 focus:border-blue-500 focus:ring-1 rounded-md px-3 py-1.5 text-sm outline-none font-mono" />
                                  <input type="text" placeholder="Matematika" value={formData.nilaiMatematika || ''} onChange={e=>setFormData({...formData, nilaiMatematika: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(false)} className="w-20 bg-white border border-blue-300 focus:border-blue-500 focus:ring-1 rounded-md px-3 py-1.5 text-sm outline-none" />
                                  <input type="text" placeholder="B.Indo" value={formData.nilaiBIndonesia || ''} onChange={e=>setFormData({...formData, nilaiBIndonesia: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(false)} className="w-20 bg-white border border-blue-300 focus:border-blue-500 focus:ring-1 rounded-md px-3 py-1.5 text-sm outline-none" />
                                  <input type="text" value={formData.kelas} onChange={e=>setFormData({...formData, kelas: e.target.value})} onKeyDown={(e)=>e.key==='Enter' && handleSave(false)} className="w-20 bg-white border border-blue-300 focus:border-blue-500 focus:ring-1 rounded-md px-3 py-1.5 text-sm outline-none" />
                                  <select value={formData.status} onChange={e=>setFormData({...formData, status: e.target.value})} onKeyUp={(e)=>e.key==='Enter' && handleSave(false)} onKeyDown={(e)=>{if(e.key==='Enter')e.preventDefault()}} className="w-28 bg-white border border-blue-300 focus:border-blue-500 focus:ring-1 rounded-md px-3 py-1.5 text-sm outline-none font-semibold text-slate-800">
                                    <option value="Lulus">Lulus</option>
                                    <option value="Tidak Lulus">Tidak Lulus</option>
                                  </select>
                                  <label className="flex items-center gap-2 text-sm text-slate-600 font-medium whitespace-nowrap cursor-pointer hover:text-slate-800">
                                     <input type="checkbox" checked={!!formData.isPranked} onChange={e=>setFormData({...formData, isPranked: e.target.checked})} className="w-4 h-4 rounded text-blue-600 border-slate-300 focus:ring-blue-500" />
                                     Prank
                                  </label>
                                  <div className="flex justify-end gap-2 ml-auto w-[80px] shrink-0">
                              ...
                              <>
                                ...
                                <td className="p-4 font-semibold text-slate-800 truncate">{student.nama}</td>
                                <td className="p-4 font-mono text-slate-500 text-sm tracking-tight">{student.nisn}</td>
                                <td className="p-4 text-slate-500 text-sm">{student.tempatLahir || '-'}</td>
                                <td className="p-4 font-mono text-slate-500 text-sm tracking-tight">{student.tglLahir || '-'}</td>
                                <td className="p-4 font-mono text-slate-500 text-sm tracking-tight">{student.noPeserta}</td>
                                <td className="p-4 font-medium text-slate-600 text-sm">{student.nilaiMatematika || '-'}</td>
                                <td className="p-4 font-medium text-slate-600 text-sm">{student.nilaiBIndonesia || '-'}</td>
                                ...
                                <td className="p-4">
                                  <div className="flex gap-2 items-center flex-wrap">
                                    ...
                                    {student.isPranked && (
                                      <span className="inline-flex px-2 py-1 rounded-lg text-[10px] font-bold uppercase tracking-wider bg-amber-100 text-amber-700 border border-amber-200">
                                        PRANK
                                      </span>
                                    )}
                                  </div>
