      ...
      if (res.ok) {
        if (data.isPranked) {
          ...
          if (prankCount === 0) {
            ...
            setPrankMessage(`Halo ${data.nama},\n\nHasil kelulusan berhasil diproses… siap mental dulu yuk coba  lagi 🫣`);
            localStorage.setItem(prankKey, (prankCount + 1).toString());
            setIsLoading(false);
            return;
          } else if (prankCount === 2) {
            setPrankMessage(`Halo ${data.nama},\n\nJangan panik… yang panik biasanya malah lulus, coba  lagi pasti hasil nya muncul 😆`);
            localStorage.setItem(prankKey, (prankCount + 1).toString());
            setIsLoading(false);
            return;
          }
                    ...
                    <div className="pt-2">
                         ...
                         <h3 className="text-xl sm:text-2xl font-bold text-slate-800 tracking-tight mb-2">
                           {searchResult.status === 'Lulus' ? (
                             <div className="flex flex-col items-center gap-2">
                               <span>SELAMAT</span>
                               <span className="text-blue-600 font-black text-2xl sm:text-3xl tracking-normal uppercase text-center leading-tight">
                                 {searchResult.nama}
                               </span>
                               <span>ANDA DINYATAKAN</span>
                             </div>
                           ) : (
                             <div className="flex flex-col items-center gap-2">
                               <span>MOHON MAAF</span>
                               <span className="text-red-600 font-black text-2xl sm:text-3xl tracking-normal uppercase text-center leading-tight">
                                 {searchResult.nama}
                               </span>
                               <span>ANDA DINYATAKAN</span>
                             </div>
                       ...
                       {searchResult.status === 'Lulus' && (
                         <motion.div
                           ...
                           className="bg-white/80 border border-slate-100 rounded-[20px] p-6 mb-8 text-left space-y-2 shadow-sm"
                         >
                           <h4 className="text-sm font-semibold text-slate-700 mb-4 uppercase tracking-wider relative inline-block">
                             Hasil Nilai TKA (Tes Kemampuan Akademik)
                           </h4>
                           <ResultRow label="Matematika" value={searchResult.nilaiMatematika || '-'} />
                           <ResultRow label="Bahasa Indonesia" value={searchResult.nilaiBIndonesia || '-'} />
                         </motion.div>
                       )}
